...
final class SanitizeTest extends TestCase
{
    ...
    {
        $this->assertEquals('Hello world', kb_sanitize_text('<b>Hello</b> <script>evil()</script>world'));
    }

    public function test_sanitize_text_trims_whitespace(): void
    {
        $this->assertEquals('Hello', kb_sanitize_text('  Hello  '));
    }

    public function test_sanitize_text_empty_string(): void
    {
        $this->assertEquals('', kb_sanitize_text(''));
    }
    ...
    public function test_sanitize_html_allows_images(): void
    {
        ...
        $this->assertStringContainsString('img', kb_sanitize_html($input));
    }
    ...
    {
        $this->assertEquals('https://example.com/path', kb_sanitize_url('https://example.com/path'));
    }

    public function test_sanitize_url_valid_http(): void
    {
        $this->assertEquals('http://example.com', kb_sanitize_url('http://example.com'));
    }

    public function test_sanitize_url_strips_javascript(): void
    {
        $this->assertEquals('', kb_sanitize_url('javascript:alert(1)'));
    }

    public function test_sanitize_url_strips_data_uri(): void
    {
        $this->assertEquals('', kb_sanitize_url('data:text/html,<script>alert(1)</script>'));
    }

    public function test_sanitize_url_trims_whitespace(): void
    {
        $this->assertEquals('https://example.com', kb_sanitize_url('  https://example.com  '));
    }

    public function test_sanitize_url_empty_for_invalid(): void
    {
        $this->assertEquals('', kb_sanitize_url('not a url'));
    }
    ...
    {
        $this->assertEquals('user@example.com', kb_sanitize_email('user@example.com'));
    }
    ...
    {
        $this->assertEquals('', kb_sanitize_email('not-an-email'));
    }

    public function test_sanitize_email_no_domain_returns_empty(): void
    {
        $this->assertEquals('', kb_sanitize_email('user@'));
    }
}
