...
final class AdminRouter
{
	...
	public function handle(): string
	{
		...
		$content = match (true) {
			...
			$route === 'menus'          => $this->menuBuilder($layout),
			$route === 'kaps'           => $this->comingSoon($layout, 'Kaps', 'kaps'),
			$route === 'templates'      => $this->templatesList($layout),
...
HTML;
	...
	private function menuBuilder(AdminLayout $layout): string
	{
		...
		if ($action !== '') {
			header('Content-Type: application/json');
			return match ($action) {
				'save_items'  => $this->menuSaveItems(),
				'delete_item' => $this->menuDeleteItem(),
				'add_items'   => $this->menuAddItems(),
				default       => json_encode(['error' => 'Unknown action']),
			};
		}
		...
		// Handle save menu settings (name, location)
		if ($this->request->method() === 'POST' && $this->request->input('_action') === 'save_menu_settings') {
			...
			if ($menuId > 0 && $menuName !== '') {
				$db->table('menus')->where('id', '=', $menuId)->update([
					'name' => $menuName,
					'location' => $menuLocation !== '' ? $menuLocation : null,
				]);
		...
		if (!empty($menuItems)) {
			foreach ($menuItems as $item) {
				...
				$itemsHtml .= '<div class="kb-menu-item-actions">';
				$itemsHtml .= '<button type="button" onclick="kbMenuIndent(' . $item->id . ')" title="Indent">→</button>';
				$itemsHtml .= '<button type="button" onclick="kbMenuOutdent(' . $item->id . ')" title="Outdent">←</button>';
				$itemsHtml .= '<button type="button" onclick="kbMenuEditItem(' . $item->id . ')" title="Edit">Edit</button>';
				$itemsHtml .= '<button type="button" class="delete" onclick="kbMenuDeleteItem(' . $item->id . ')" title="Remove">×</button>';
				$itemsHtml .= '</div></div>';
			}
		...
		// Pages list for "Add to menu"
		$pages = $db->table('posts')->where('type', '=', 'page')->where('status', '=', 'published')->orderBy('title', 'ASC')->get();
		$pagesCheckboxes = '';
		foreach ($pages as $page) {
			$pagesCheckboxes .= '<label><input type="checkbox" value="' . $page->id . '" data-label="' . $e($page->title) . '" data-url="/' . $e($page->slug) . '"> ' . $e($page->title) . '</label>';
		}
		...
		foreach ($postsList as $post) {
			$postsCheckboxes .= '<label><input type="checkbox" value="' . $post->id . '" data-label="' . $e($post->title) . '" data-url="/' . $e($post->slug) . '"> ' . $e($post->title) . '</label>';
		}
		...
		$catsCheckboxes = '';
		foreach ($cats as $cat) {
			$catsCheckboxes .= '<label><input type="checkbox" value="' . $cat->id . '" data-label="' . $e($cat->name) . '" data-url="/category/' . $e($cat->slug) . '"> ' . $e($cat->name) . '</label>';
		}
		...
		if ($selectedMenu) {
			$menuSettingsHtml = '<form method="POST" action="/kb-admin/menus?menu_id=' . $selectedMenuId . '"><input type="hidden" name="_action" value="save_menu_settings"><input type="hidden" name="menu_id" value="' . $selectedMenuId . '">'
				. '<div style="display:flex;gap:10px;align-items:end;margin-bottom:16px;flex-wrap:wrap;">'
				. '<div class="kb-form-group" style="margin-bottom:0;"><label class="kb-label">Menu name</label><input type="text" name="menu_name" class="kb-input" value="' . $e($selectedMenu->name) . '" style="max-width:240px;"></div>'
				. '<div class="kb-form-group" style="margin-bottom:0;"><label class="kb-label">Location</label><select name="menu_location" class="kb-select" style="max-width:220px;">' . $locationOptions . '</select></div>'
				. '<button type="submit" class="kb-btn kb-btn-primary kb-btn-sm">Save menu</button>'
				. '</div></form>';

			$menuContentHtml = '<div id="kb-menu-items-list">' . $itemsHtml . '</div>'
				. '<div style="margin-top:12px;display:flex;gap:8px;justify-content:space-between;">'
				. '<span class="kb-menu-unsaved" id="kb-menu-unsaved" style="display:none;">Unsaved order changes</span>'
				. '<div style="display:flex;gap:8px;">'
				. '<button type="button" class="kb-btn kb-btn-primary kb-btn-sm" id="kb-menu-save-order" style="display:none;" onclick="kbMenuSaveOrder()">Save order</button>'
		...
		<div class="kb-menu-selector">
			<select class="kb-select" id="kb-menu-select" onchange="window.location='/kb-admin/menus?menu_id='+this.value" style="max-width:260px;">
				{$menuOptions}
			</select>
			<span style="color:var(--kb-text-hint);font-size:13px;">or</span>
			<form method="POST" action="/kb-admin/menus" style="display:flex;gap:6px;align-items:center;">
				<input type="hidden" name="_action" value="create_menu">
				<input type="text" name="menu_name" class="kb-input" placeholder="New menu name..." style="max-width:200px;" required>
				<button type="submit" class="kb-btn kb-btn-primary kb-btn-sm">Create</button>
			</form>
		</div>
		...
		<div class="kb-grid-2" style="grid-template-columns:280px 1fr;">
			<div class="kb-stack">
				<div class="kb-menu-add-panel">
					<div class="kb-menu-add-header" onclick="this.nextElementSibling.classList.toggle('open')">Pages <span>▸</span></div>
					<div class="kb-menu-add-body">
						<div class="kb-menu-add-list" id="kb-add-pages">{$pagesCheckboxes}</div>
						<button type="button" class="kb-btn kb-btn-outline kb-btn-sm" onclick="kbMenuAddChecked('kb-add-pages', 'page')">Add to menu</button>
					</div>
				</div>
				<div class="kb-menu-add-panel">
					<div class="kb-menu-add-header" onclick="this.nextElementSibling.classList.toggle('open')">Posts <span>▸</span></div>
					<div class="kb-menu-add-body">
						<div class="kb-menu-add-list" id="kb-add-posts">{$postsCheckboxes}</div>
						<button type="button" class="kb-btn kb-btn-outline kb-btn-sm" onclick="kbMenuAddChecked('kb-add-posts', 'post')">Add to menu</button>
					</div>
				</div>
				<div class="kb-menu-add-panel">
					<div class="kb-menu-add-header" onclick="this.nextElementSibling.classList.toggle('open')">Categories <span>▸</span></div>
					<div class="kb-menu-add-body">
						<div class="kb-menu-add-list" id="kb-add-cats">{$catsCheckboxes}</div>
						<button type="button" class="kb-btn kb-btn-outline kb-btn-sm" onclick="kbMenuAddChecked('kb-add-cats', 'category')">Add to menu</button>
					</div>
				</div>
				<div class="kb-menu-add-panel">
					<div class="kb-menu-add-header" onclick="this.nextElementSibling.classList.toggle('open')">Custom link <span>▸</span></div>
					<div class="kb-menu-add-body open">
						<div class="kb-form-group"><label class="kb-label">URL</label><input type="url" id="kb-custom-url" class="kb-input" placeholder="https://"></div>
						<div class="kb-form-group"><label class="kb-label">Label</label><input type="text" id="kb-custom-label" class="kb-input" placeholder="Link text"></div>
						<button type="button" class="kb-btn kb-btn-outline kb-btn-sm" onclick="kbMenuAddCustom()">Add to menu</button>
					</div>
				</div>
			</div>
			<div>
				<div class="kb-card">
					<div class="kb-card-header"><h3>Menu structure</h3></div>
					<div class="kb-card-body" style="min-height:100px;">
						{$menuContentHtml}
					</div>
				</div>
			</div>
		...
		<script>
		const MENU_ID = {$selectedMenuId};
		let menuSortable = null;
		let menuDirty = false;

		document.addEventListener('DOMContentLoaded', () => {
			const list = document.getElementById('kb-menu-items-list');
			if (!list || !MENU_ID) return;

			menuSortable = new KB.Sortable(list, {
				itemSelector: '.kb-sortable-item',
				handleSelector: '.kb-drag-handle',
				nestable: true,
				maxDepth: 2,
				onReorder: () => {
					menuDirty = true;
					document.getElementById('kb-menu-unsaved').style.display = '';
					document.getElementById('kb-menu-save-order').style.display = '';
				},
			});
			menuSortable.refreshIndents();
		});

		function kbMenuIndent(id) {
			const el = document.querySelector('[data-id="' + id + '"]');
			if (el && menuSortable) { menuSortable.indent(el); menuDirty = true; document.getElementById('kb-menu-unsaved').style.display = ''; document.getElementById('kb-menu-save-order').style.display = ''; }
		}
		function kbMenuOutdent(id) {
			const el = document.querySelector('[data-id="' + id + '"]');
			if (el && menuSortable) { menuSortable.outdent(el); menuDirty = true; document.getElementById('kb-menu-unsaved').style.display = ''; document.getElementById('kb-menu-save-order').style.display = ''; }
		}

		async function kbMenuSaveOrder() {
			const items = [...document.querySelectorAll('#kb-menu-items-list .kb-sortable-item')].map((el, i) => ({
			...
			try {
				const res = await fetch('/kb-admin/menus?action=save_items', {
					method: 'POST',
					headers: { 'Content-Type': 'application/json' },
					body: JSON.stringify({ menu_id: MENU_ID, items }),
				});
				const data = await res.json();
				if (data.success) {
					KB.toast('success', 'Menu order saved.');
					menuDirty = false;
					document.getElementById('kb-menu-unsaved').style.display = 'none';
					document.getElementById('kb-menu-save-order').style.display = 'none';
				} else {
					KB.toast('error', data.error ?? 'Failed to save.');
				}
			} catch { KB.toast('error', 'Network error.'); }
		}

		async function kbMenuDeleteItem(id) {
			if (!window.confirm('Remove this menu item?')) return;
			try {
				const res = await fetch('/kb-admin/menus?action=delete_item', {
					method: 'POST',
					headers: { 'Content-Type': 'application/json' },
				...
				if (data.success) {
					...
					KB.toast('success', 'Item removed.');
				}
			} catch { KB.toast('error', 'Network error.'); }
		}

		function kbMenuEditItem(id) {
			const el = document.querySelector('[data-id="' + id + '"]');
			if (!el) return;
			const labelEl = el.querySelector('.kb-menu-item-label');
		...
		async function kbMenuAddChecked(containerId, type) {
			if (!MENU_ID) return;
			...
			const items = checked.map(cb => ({
				type,
				object_id: parseInt(cb.value),
				label: cb.dataset.label,
				url: cb.dataset.url,
			}));

			try {
				const res = await fetch('/kb-admin/menus?action=add_items', {
					method: 'POST',
					headers: { 'Content-Type': 'application/json' },
					body: JSON.stringify({ menu_id: MENU_ID, items }),
				});
		...
		async function kbMenuAddCustom() {
			if (!MENU_ID) return;
			const url = document.getElementById('kb-custom-url')?.value.trim();
			const label = document.getElementById('kb-custom-label')?.value.trim();
			if (!url || !label) { KB.toast('error', 'URL and label are required.'); return; }

			try {
				const res = await fetch('/kb-admin/menus?action=add_items', {
					method: 'POST',
					headers: { 'Content-Type': 'application/json' },
					body: JSON.stringify({ menu_id: MENU_ID, items: [{ type: 'custom', label, url }] }),
				});
				const data = await res.json();
				if (data.success) {
					window.location.reload();
		...
		window.addEventListener('beforeunload', (e) => {
			if (menuDirty) { e.preventDefault(); e.returnValue = ''; }
		});
		</script>
HTML;
	...
	private function menuSaveItems(): string
	{
		if ($this->request->method() !== 'POST') {
			return json_encode(['error' => 'Method not allowed']);
		}
		...
		// Build parent_id mapping from depth changes
		// Items with depth 0 have no parent
		// Items with depth > 0 have the nearest preceding item with depth-1 as parent
		$orderedIds = [];
		foreach ($items as $item) {
			$orderedIds[] = ['id' => (int) $item['id'], 'depth' => (int) $item['depth']];
		}
		...
		foreach ($items as $i => $item) {
			...
			if ($depth > 0) {
				...
				for ($j = $i - 1; $j >= 0; $j--) {
					if ((int) $items[$j]['depth'] === $depth - 1) {
						$parentId = (int) $items[$j]['id'];
						break;
					}
				}
			}

			// Also update the label if it was edited inline
			$updateData = [
	...
	private function menuDeleteItem(): string
	{
		if ($this->request->method() !== 'POST') {
			return json_encode(['error' => 'Method not allowed']);
		}
	...
	private function menuAddItems(): string
	{
		if ($this->request->method() !== 'POST') {
			return json_encode(['error' => 'Method not allowed']);
		}
		...
		if ($last) {
			$maxOrder = (int) $last->sort_order + 1;
		}
		...
		foreach ($items as $item) {
			$db->table('menu_items')->insert([
				...
				'url'         => $item['url'] ?? '#',
				'object_type' => in_array($item['type'] ?? '', ['page', 'post', 'category']) ? $item['type'] : null,
	...
	private function templatesList(AdminLayout $layout): string
	{
		...
		if ($this->request->method() === 'POST' && $this->request->input('_action') === 'activate') {
			...
			if ($slug !== '' && is_dir($templatesDir)) {
				...
				if ($existing) {
					$db->table('options')->where('option_key', '=', 'active_template')->update(['option_value' => $slug]);
				} else {
					$db->table('options')->insert(['option_key' => 'active_template', 'option_value' => $slug, 'autoload' => 1]);
				}
				if ($this->app->has('cache')) {
					$this->app->cache()->clearPageCache();
				}
				...
				$layout->addNotice('error', 'Template not found.');
			}
		}
		...
		if (is_dir($templatesPath)) {
			foreach (new \DirectoryIterator($templatesPath) as $dir) {
				...
				$manifest = $this->loadTemplateManifest($templatesPath . '/' . $slug);
				$manifest['slug'] = $slug;
				...
				$manifest['has_screenshot'] = file_exists($templatesPath . '/' . $slug . '/screenshot.png')
					|| file_exists($templatesPath . '/' . $slug . '/screenshot.jpg');
		...
		foreach ($templates as $t) {
			if ($t['is_active']) { $activeTemplate = $t; break; }
		}

		// Build active template detail panel
		$activeHtml = '';
		if ($activeTemplate) {
			$activeHtml = $this->renderTemplateDetail($activeTemplate, $e, true);
		}

		// Build available templates grid
		$gridHtml = '';
		foreach ($templates as $t) {
			...
			$actionBtn = '';
			if (!$isActive) {
				$actionBtn = '<form method="POST" action="/kb-admin/templates" style="display:inline;"><input type="hidden" name="_action" value="activate"><input type="hidden" name="template_slug" value="' . $e($t['slug']) . '"><button type="submit" class="kb-btn kb-btn-primary kb-btn-sm" onclick="return confirm(\'Activate ' . $e($t['name']) . '?\')">Activate</button></form>';
			}

			$gridHtml .= '<div class="kb-template-card" style="border:2px solid var(--kb-border);border-radius:var(--kb-radius);overflow:hidden;position:relative;background:var(--kb-card);transition:border-color 0.15s;' . $borderStyle . '">';
			$gridHtml .= $badge;
			$gridHtml .= $screenshotHtml;
			$gridHtml .= '<div style="padding:12px 14px;">';
			$gridHtml .= '<div style="font-weight:700;font-size:14px;margin-bottom:2px;">' . $e($t['name']) . '</div>';
			$gridHtml .= '<div style="font-size:12px;color:var(--kb-text-hint);margin-bottom:8px;">v' . $e($t['version']) . ' by ' . $e($t['author']) . '</div>';
			$gridHtml .= '<div style="display:flex;gap:6px;align-items:center;">';
			$gridHtml .= $actionBtn;
			$gridHtml .= '<button type="button" class="kb-btn kb-btn-outline kb-btn-sm" onclick="kbTemplateDetail(\'' . $e($t['slug']) . '\')">Details</button>';
			$gridHtml .= '</div></div></div>';
		}

		if ($gridHtml === '') {
			$gridHtml = '<div class="kb-empty"><h3>No templates found</h3><p>Place template folders in kb-content/templates/</p></div>';
		}
		...
		<div style="display:grid;grid-template-columns:1fr 320px;gap:20px;">
			...
			<div>
				<div class="kb-card" id="kb-template-detail">
					<div class="kb-card-header"><h3>Active template</h3></div>
					<div class="kb-card-body">
						{$activeHtml}
					</div>
				</div>
		...
		<script>
		function kbTemplateDetail(slug) {
			// For now, scroll to the detail panel and highlight the card
			// Full AJAX detail loading can come later
			const card = document.querySelector('.kb-template-card');
			const detail = document.getElementById('kb-template-detail');
...
HTML;
	...
	private function renderTemplateDetail(array $t, \Closure $e, bool $isActive): string
	{
		$html = '';
		...
		$html .= '<div style="font-size:16px;font-weight:700;margin-bottom:4px;">' . $e($t['name']) . '</div>';
		if ($isActive) {
			$html .= '<span class="kb-badge kb-badge-published">Active</span>';
		}
		$html .= '</div>';
		...
		$rows = [
			'Version' => 'v' . $t['version'],
			'Author' => $t['author_url'] ? '<a href="' . $e($t['author_url']) . '" target="_blank">' . $e($t['author']) . '</a>' : $e($t['author']),
			'License' => $t['license'],
			'Files' => $t['file_count'] . ' template file' . ($t['file_count'] !== 1 ? 's' : ''),
		];

		if (!empty($t['locations'])) {
			$locs = [];
			foreach ($t['locations'] as $key => $label) {
				$locs[] = $e($label);
			}
			$rows['Menu locations'] = implode(', ', $locs);
		}

		foreach ($rows as $label => $value) {
			$html .= '<div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid var(--kb-border);">';
			$html .= '<span style="color:var(--kb-text-hint);">' . $e($label) . '</span>';
			$html .= '<span style="color:var(--kb-text);">' . $value . '</span>';
			$html .= '</div>';
		}
		$html .= '</div>';
	...
	private function loadTemplateManifest(string $templatePath): array
	{
		$defaults = [
			...
			'author_url'      => '',
			'license'         => 'Proprietary',
			'requires_kreblu' => '1.0.0',
			'locations'       => [],
			'settings'        => [],
		];

		$jsonPath = $templatePath . '/theme.json';
